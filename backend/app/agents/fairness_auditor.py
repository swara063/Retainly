...
class FairnessAuditorAgent(BaseAgent):
    ...
    def run(self, context: dict) -> dict:
        ...
        y_test = pd.Series(artifacts["y_test"], index=X_test.index)
        ...
        for col in sensitive:
            group_df = pd.DataFrame({"group": X_test[col].astype(str), "actual": y_test, "predicted": y_pred})
            rows = []
            for group, part in group_df.groupby("group"):
                ...
                rows.append({"group": group, "count": int(len(part)), "prediction_rate": prediction_rate, "false_positive_rate": false_positive_rate, "false_negative_rate": false_negative_rate})
            if rows:
                pred_gap = max(r["prediction_rate"] for r in rows) - min(r["prediction_rate"] for r in rows)
                fp_gap = max(r["false_positive_rate"] for r in rows) - min(r["false_positive_rate"] for r in rows)
