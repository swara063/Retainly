...
def build_pdf_report(dataset_id: str, results: dict) -> str:
    ...
    story.append(
        _kv_table(
            [
                ...
                ("Recall / F1 / ROC-AUC", f"{_fmt_num(recall)} / {_fmt_num(f1)} / {_fmt_num(roc_auc)}"),
                ("PR-AUC (if available)", _fmt_num(pr_auc)),
    ...
    metric_rows = [
        ("Selected model", selected_model),
        ("Recall", _fmt_num(recall)),
        ("Precision", _fmt_num(metrics.get("precision"))),
        ("F1", _fmt_num(f1)),
        ("ROC-AUC", _fmt_num(roc_auc)),
        ("PR-AUC", _fmt_num(pr_auc)),
        ("Recall @ top 10%", _fmt_num(metrics.get("recall_at_top_10_percent"))),
        ("Recall @ top 20%", _fmt_num(metrics.get("recall_at_top_20_percent"))),
